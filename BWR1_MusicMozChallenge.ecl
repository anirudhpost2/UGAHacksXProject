...
nameSort := SORT(MozMusic, name);
OUTPUT(CHOOSEN(nameSort, 50, 1));
...
DepecheMode := MozMusic( name='Depeche Mode' );
  DedupDepeche := DEDUP(DepecheMode,name);
OUTPUT(MozMusic, DedupDepeche);
...
//Display count and result 
 ...
 MyWay := MozMusic( title='My Way' );
OUTPUT(MozMusic, MyWay);
...
imaginalDisk := MozMusic(formats = 'CD');
OUTPUT(imaginalDisk);
...
youTwo := MozMusic(name = 'U2');
youTwoSort := SORT(youTwo, name);
youToo := OUTPUT(youTwoSort, NAMED('Asc_name');
countU := COUNT(youToo);
OUTPUT(countU);
...
guest := MozMusic(guestMusicians = 'not blank');
